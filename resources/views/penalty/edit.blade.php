...
<div class="container">
    ...
    <form action="/penalty/{{$lists['penalties'] -> id}}" method="POST">
        @csrf
        @method('PUT')
        <div class="row row-cols-lg-3 g-3 align-items-center mb-3">
              ...
              <select class="form-select @error('profile_id') is-invalid @enderror" id="profile_id" name="profile_id" >
                <option selected>-</option>
                    @foreach ($lists['Profiles'] as $profile)
                        <option value="{{$profile -> id}}" {{ $profile -> id == $lists['penalties'] -> profile_id ? 'selected' : ''}}>{{$profile -> first_name . ' ' . $profile -> father_name . ' ' . $profile -> last_name}}</option>
                    @endforeach
              </select>
              @error('profile_id')
            ...
            <div class="col-12">
              ...
              <select class="form-select @error('penalty_id') is-invalid @enderror" id="penalty_id" name="penalty_id">
                ...
                @foreach ($lists['penalty_names'] as $penalty_name)
                    <option value="{{$penalty_name -> id}}" {{ $penalty_name -> id == $lists['penalties'] -> penalty_id ? 'selected' : ''}}>{{$penalty_name -> penalty_name }}</option>
                @endforeach
              </select>
              @error('penalty_id')
                <span class="invalid-feedback" role="alert">
            ...
            <div class="col-12">
                ...
                <input type="text" class="form-control @error('penalty_date') is-invalid @enderror" id="penalty_date" name="penalty_date" value="{{date('d-m-Y', strtotime($lists['penalties'] -> penalty_date))}}">
                @error('penalty_date')
                <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
                </span>
            ...
            <div class="col-12">
              ...
              <select class="form-select @error('penalty_source') is-invalid @enderror" id="penalty_source" name="penalty_source">
                  ...
                  <option value="المركز الرئيسي" {{ $lists['penalties'] -> penalty_source == 'المركز الرئيسي' ? 'selected' : ''}}>المركز الرئيسي</option>
              </select>
            ...
            <div class="col-12">
                ...
                <input type="text" class="form-control @error('penalty_reason') is-invalid @enderror" id="penalty_reason" name="penalty_reason" value="{{$lists['penalties'] -> penalty_reason}}">
