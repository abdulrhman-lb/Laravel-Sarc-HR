        ...
        <table class="table table-bordered">
            ...
            @foreach ($penalties as $penalty)        
                <tr class="pt-3 ">
                    ...
                    <td class="centered-content">{{$penalty -> profile -> first_name . ' ' . $penalty -> profile -> father_name . ' ' . $penalty -> profile -> last_name}}</td>
                    ...
                    <td class="centered-content">
                        <form action="{{action('PenaltyController@destroy', $penalty -> id)}}" method="POST">
                            @csrf
                            @method("DELETE")
                            <a href="/penalty/{{$penalty -> id}}"><button type="button" class="btn btn-primary my-1"><i class="fa fa-eye"></i></button></a>
                            <a href="/penalty/{{$penalty -> id}}/edit"><button type="button" class="btn btn-success my-1"><i class="fa fa-edit"></i></button></a>
